...
def callback(data):
    global start,p_desired,robot_state,robot_state_history,i,obstacle_to_be_passed,c

    array_rec = np.array(data.data).reshape((4, 2))
    # predict the obstacles' position in future
    obstacle_predictions = predict_obstacle_positions(array_rec)
    xref = compute_xref(robot_state, p_desired,
                        HORIZON_LENGTH, NMPC_TIMESTEP)
    # compute velocity using nmpc
    vel, velocity_profile = compute_velocity(
